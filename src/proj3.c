...
extern int yylineno;
extern char str_table[];      /* string table in table.c */
...
void error_msg(int type, int action, int id, int seq){
    printf("Semantic Error--line: %d, ", yylineno-1);
    switch (type)
    {
        case STACK_OVERFLOW:
...
int InsertEntry(int id){
    static int main_symbol_idx = 0, has_main = 0;
    if (IsAttr(main_symbol_idx, KIND_ATTR) && GetAttr(main_symbol_idx, KIND_ATTR)==PROCE)
        // This means function main is already defined.
        has_main = 1;
    /* id is already declared in the current block */
    if ( LookUpHere(id) || (has_main && strcmp("main", str_table+id)==0))    {
    ...
    if (strcmp("main", str_table+id)==0)
        main_symbol_idx = st_top+1;
...
int LookUpField(int st_ptr, int id){
